...
public static class ClaudeSystemPrompt
{
    ...
    public static string Embedded(ClaudeLaunchOptions o)
    {
        ...
        sb.AppendLine("- Before writing a common mechanic from scratch, call search_cookies. The CookieJar is this team's library of ready-made modules, and install_cookie copies one into the project, builds it, and returns its AGENT.md and next steps in the result -- follow those rather than re-deriving the module or reading its files. A cookie from a jar the user has not trusted is refused: say so and let them approve it in the Cookie Jar panel rather than working around it. When you finish something a second game would want, bake_cookie it back, with instructions that say how to wire it up.");
