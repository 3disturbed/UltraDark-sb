...
public static class ActionEvaluator
{
    ...
    /// fires. The same file has to work in both engines, so both sides normalise through the same
    /// table.
    /// </remarks>
    public static bool TryParseKey(string? name, out Keys key) => KeyNames.TryParse(name, out key);
