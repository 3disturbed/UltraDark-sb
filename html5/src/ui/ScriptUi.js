// -----------------------------------------------------------------------------
// ScriptUi — screen-space UI a game script can actually build.
//
// Named for its C# mirror, SexyBiscuit.Engine/UI/ScriptUi.cs. The richer retained
// tree that UiNode/UiLayout/UiFocus build is UiCanvas.js, next door.
//
...
export class ScriptUi {
    ...
    add(kind, options) {
        if (!KINDS.has(kind)) throw new Error(`ScriptUi: unknown element kind "${kind}"`);
